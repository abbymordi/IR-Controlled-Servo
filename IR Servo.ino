#define ON_OFF_KEY 0xFFA25D  
#define TOGGLE_COLOR_KEY 0xFF629D
#define BRIGHTNESS_UP_KEY 0xFFE21D
#define BRIGHTNESS_DOWN_KEY 0xFF22DD
#define CIRCLE_KEY 0xFF02FD
  ...
  if (irrecv.decode(&results)) {
    ...
    if (isOn == true) {
      if (results.value == TOGGLE_COLOR_KEY)  // Toggle active colors
        ...
        circle();
      else if (results.value == CUSTOM_FUNCTION_KEY) {
        // Move the servo to position 90 degrees
        Serial.println("Moving servo to 90 degrees");
        myservo.write(90);  // Example servo movement
        delay(1000);
        myservo.write(0);   // Move it back to 0 degrees after a delay
